...
版本号遵循 [语义化版本](https://semver.org/lang/zh-CN/)，格式为 `主版本.次版本.补丁号`：
...
> ⚠️ **血泪教训：版本号的每一段都是独立整数，不是小数！**
>
> 你可能以为 `2.0.21` 是 `2.0.2` 的子版本（像 2.0.2.1），但**不是**。
> 在 semver 中 `2.0.21` = 补丁号 **21**，而 `2.0.3` = 补丁号 **3**。
>
> 所以 **`2.0.21` > `2.0.3`**，如果你从 `2.0.22` 发布到 `2.0.3`，
> 对于所有版本比较工具（Tauri updater、npm、cargo）来说这是**降级**，
> 已安装 v2.0.22 的用户将**永远检测不到** v2.0.3 的更新。
>
> **正确做法**：
> - 想发小修补？顺序递增：`2.0.22` → `2.0.23` → `2.0.24`
> - 想发功能更新？升次版本：`2.0.22` → `2.1.0`
> - 不要跳回小数字！`2.0.22` → ~~`2.0.3`~~ ❌
...
| 用户检测不到更新（latest.json 正常） | **版本号回退了！** 新 Release 的版本号可能比用户已安装的版本**小**（如从 2.0.22 发布到 2.0.3）。见 Step 1 的 ⚠️ 警告，修复方法：发一个版本号更大的新版本 |
